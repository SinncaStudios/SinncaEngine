...
namespace sinnca
{
	...
	float vec3_dot_vec3( v3 *v0, v3 *v1 )
	{
		return ( v0->x * v1->x ) +
		( v0->y * v1->y ) +
		( v0->z * v1->z );
	...
	float vec3_length( v3 *v )
	{ return sqrtf( vec3_dot( v ) ); }

	float normalize(v3* d, v3* v)
	{
	...
	{
		float s = sinf(next->w * (pi / 180.f));
		float c = cosf(next->w * (pi / 180.f));
		
		float xx, yy, zz, xy, yz, zx, xs, ys, zs, cc;
		
		m4 wMat;
		v3 v = {next->x, next->y, next->z};
		snLoadIdentity(&wMat);
		
		if (!next->w || !normalize(&v, &v))
		{
			return;
		}
		...
		multiply(destination, first, &wMat);
